...
set -x 
IOS_DIR="$(pwd)/../ios"
DYLIB_PATH="$(pwd)/../ios/native_libs"
TMP_DIR="${IOS_DIR}/tmp"
...
write_info_plist() {
    ...
    if [[ "x$target" = "xios-arm64-simulator" ]]; then
...
create_framework() {
    ...
    input_dylib="${DYLIB_PATH}/${target}/${wallet}_libwallet2_api_c.dylib"
...
for i in "${!wallets[@]}"; do
    ...
    create_framework "$wallet" "$framework_name" "ios-arm64" "$device_out"
    sleep 5
    create_framework "$wallet" "$framework_name" "ios-arm64-simulator" "$simulator_out"
