...
libs_android_build:
	[[ ! "x$(uname)" == "xLinux" ]] && exit 1 # Only Linux hosts can build for android, try $(MAKE) libs_android_download
...
libs_ios_build:
	[[ ! "x$(uname)" == "xDarwin" ]] && exit 1 # Only Darwin hosts can build for iOS, try $(MAKE) libs_ios_download
